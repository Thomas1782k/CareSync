...
<% response.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
    ...
    <form class="p-2 p-md-1 row g-3 border rounded-3 bg-light">
        <div class="form-floating mb-3">
            ...
            <label for="patientIdSelect">Patient ID</label>
        </div>

        <div class="form-floating mb-3">
            <select name="pname" id="patientNameSelect" class="form-select">
                <c:forEach items="${patients}" var="pat">
                    <option value="${pat.getFirstName()}">${pat.getFirstName()} - ${pat.getPid()} - ${pat.getDoctor()}</option>
                </c:forEach>
            </select>
            <label for="patientNameSelect">Patient Name</label>
        </div>

        <div class="form-floating mb-3">
            <input type="datetime-local" name="appointmentDate" class="form-control" id="appointmentDateInput" placeholder="Date">
            <label for="appointmentDateInput">Date</label>
        </div>

        <div class="form-floating mb-3">
            <input type="text" name="symptoms" class="form-control" id="symptomsInput" placeholder="Symptoms">
            <label for="symptomsInput">Symptoms</label>
        </div>

        <div class="form-floating mb-3">
            <select name="doctor" id="doctorsAvailableSelect" class="form-select">
                <c:forEach items="${doctors}" var="doctor">
                    <option value="${doctor.getDid()}">${doctor.getFirstName()} - ${doctor.getSpeciality()}</option>
                </c:forEach>
            </select>
            <label for="doctorsAvailableSelect">Doctors Available</label>
        </div>
